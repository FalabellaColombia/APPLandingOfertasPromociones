      ...
      setAllProducts((prev) => {
        ...
        switch (payload.eventType) {
          ...
          case "UPDATE":
            if (payload.new) {
              ...
              const isRebalance =
                updated.filter(
                  (p) => p.orderSellout % 100 === 0 && p.orderSellout > 0
                ).length > 3; // Si varios productos tienen valores como 100, 200, etc.
              ...
              if (isRebalance) {
               console.log("update2")
                ...
                payload.new.orderSellout !== payload.old?.orderSellout
              ) {
        ...
        const filteredProducts =
          activeButton === VIEW_LISTADO
            ? getVisibleProducts(updated)
            : updated.filter((p) => p.isProductHidden);

        // Asegurar ordenamiento
        const sortedProducts = [...filteredProducts].sort(
          (a, b) => a.orderSellout - b.orderSellout
        );
